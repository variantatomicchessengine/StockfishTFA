...
#ifdef ATOMIC
  if((square<KING>(sideToMove) == SQ_NONE ||
...
#ifdef ATOMIC
  if(type_of(piece_on(from)) == KING && type_of(m) != CASTLING)
  {
      Square ksq = square<KING>(~us);
      Square to = to_sq(m);
...
#ifdef ATOMIC
  if(captured) // Remove the blast piece(s)
  {
...
#ifdef ATOMIC
  if(st->capturedType) // Restore the blast piece(s)
...
bool Position::pos_is_ok(int* failedStep) const {
  ...
  for (int step = Default; step <= (Fast ? Default : Castling); step++)
  {
...
#ifdef ATOMIC
              || ((wksq != SQ_NONE) && piece_on(wksq) != W_KING)
#else
      ...
      if (step == King)
      {
          ...
          else if((attacks_from<KING>(square<KING>(~sideToMove)) & square<KING>(sideToMove)))
          {
          } else
#endif
